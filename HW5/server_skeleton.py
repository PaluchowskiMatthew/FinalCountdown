...
class RiffleServer:
    ...
    def recv_client_shared_key(self, skey_encrypted):
        ...
        # TODO: Implement!
        cip = binascii.unhexlify(skey_encrypted)
        iv = cip[:AES_IV_SIZE]
        pub_key = self.rsa_key_pair.publickey().exportKey()
        client_shared_key = AES.new(pub_key, AES_MODE, iv).decrypt(cip[AES_IV_SIZE:])
        pass
    ...
    def recv_chat_msg_client(self, msg_encrypted):
        ...
        iv = cip[:AES_IV_SIZE]
        cip = AES.new(client_shared_key, AES_MODE, iv).decrypt(cip[AES_IV_SIZE:])

        # If there is next server send the decrypted message to it,
        # else this is the only server and just store the message
        # TODO: insert your code here!
        servers = [server for server in self.all_servers if server['server_id'] != self.server_id]
        if len(servers) == 0: #no servers, store message
            if self.chat_messages:
                max_key = max(chat_messages.keys(), key=int)+1 # get max key
            else:
                max_key = 0
    ...
    def recv_chat_msg_server(self, msg_encrypted, sender):
        ...
        # TODO: insert your code here!
        if sender.server_id != self.server_id-1:
            return
    ...
    def recv_chat_msg_plain(self, msg):
        ...
        if self.chat_messages:
            ...
            max_key = 0
        self.chat_messages[max_key] = msg
