...
def get_db1():
    ...
    if db1 is None:
        db1 = pymysql.connect('localhost', config['USERNAME1'],
                              config['USER_PWD1'], config['DATABASE1'])
        cursor1 = db1.cursor()
...
def get_db2():
    ...
    if db2 is None:
        db2 = g._database = pymysql.connect('localhost',
                config['USERNAME2'], config['USER_PWD2'],
                config['DATABASE2'])
        cursor2 = db2.cursor()
...
def messages():
    with get_db2().cursor() as cursor:
        ...
        if request.method == 'GET':
            cursor.execute(sql)
            return render_template('messages.html', show=False,
        ...
        if len(results) == 0:
            return render_template('messages.html', show=True,
                                   exists=False)
        else:
            return render_template('messages.html', show=True,
                                   exists=True)
...
def contact():
    ...
    if message == '':
        ...
        return render_template(template, error=error, expl=expl)
    sql = \
        'INSERT INTO contact_messages (name,mail,message) VALUES (%s,%s,%s);'
    with get_db1().cursor() as cursor:
